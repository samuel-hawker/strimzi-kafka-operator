...
public class KafkaCrdOperatorIT {
    ...
    public void testUpdateStatus(VertxTestContext context) {
        ...
        PlatformFeaturesAvailability.create(vertx, client)
            ...
            .compose(pfa -> {
                ...
                return kafkaOperator.reconcile(namespace, RESOURCE_NAME, getResource());
            })
            .setHandler(context.succeeding())
            .compose(rrCreated -> {
                Kafka newStatus = new KafkaBuilder(kafkaOperator.get(namespace, RESOURCE_NAME))
                        .withNewStatus()
                ...
                return kafkaOperator.updateStatusAsync(newStatus);
            })
            .setHandler(context.succeeding())

            .compose(rrModified -> kafkaOperator.getAsync(namespace, RESOURCE_NAME))
            .setHandler(context.succeeding(modifiedKafka -> context.verify(() -> {
                assertThat(modifiedKafka.getStatus().getConditions().get(0).getType(), is("Ready"));
                assertThat(modifiedKafka.getStatus().getConditions().get(0).getStatus(), is("True"));
            })))

            .compose(rrModified -> {
                log.info("Deleting resource");
                return kafkaOperator.reconcile(namespace, RESOURCE_NAME, null);
            })
    ...
    public void testUpdateStatusThrowsKubernetesExceptionIfResourceUpdatedPrior(VertxTestContext context) {
        ...
        PlatformFeaturesAvailability.create(vertx, client)
            ...
            .setHandler(context.succeeding())
            .compose(rr -> {
                Kafka currentKafka = kafkaOperator.get(namespace, RESOURCE_NAME);

                Kafka updated = new KafkaBuilder(currentKafka)
                        .editSpec()
                        .editKafka()
                        .addToConfig("xxx", "yyy")
                        .endKafka()
                ...
                Kafka newStatus = new KafkaBuilder(currentKafka)
                        .withNewStatus()
                        .withConditions(new ConditionBuilder()
                                .withType("Ready")
                                .withStatus("True")
